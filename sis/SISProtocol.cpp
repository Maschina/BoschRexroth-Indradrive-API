...
void SISProtocol::get_parameter_attributes(TGM::SercosParamVar _paramvar, const USHORT &_paramnum, UINT8& _scalefactor, size_t& _datalen)
{
	...
	if (sercos_attribute.Bits.DataLen == TGM::Datalen_4ByteList) _datalen = 4;
	if (sercos_attribute.Bits.DataLen == TGM::Datalen_8ByteList) _datalen = 8;
	if (sercos_attribute.Bits.DataLen == TGM::Datalen_2ByteParam) _datalen = 2;
	if (sercos_attribute.Bits.DataLen == TGM::Datalen_4ByteParam) _datalen = 4;
	if (sercos_attribute.Bits.DataLen == TGM::Datalen_8ByteParam) _datalen = 8;
