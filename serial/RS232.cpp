//	RS232.cpp - Implementation of the CSerial class
//
...
CSerial::~CSerial ()
{
	...
	if (m_hFile)
	{
		...
		// Display a warning
		//throw ExceptionGeneric("CSerial::Open", __FILE__, __LINE__, m_lLastError, "Serial port not closed", true);
	}
}
...
void CSerial::Open (LPCTSTR lpszDevice, DWORD dwInQueue, DWORD dwOutQueue, bool fOverlapped)
{
	...
	// Open the device
	m_hFile = ::CreateFile(lpszDevice,							// address of name of the communications device
					GENERIC_READ | GENERIC_WRITE,				// access (read-write) mode
					0,											// share mode
					0,											// address of security descriptor
					OPEN_EXISTING,								// how to create
					fOverlapped ? FILE_FLAG_OVERLAPPED : 0,		// file attributes
					0);											// handle of file with attributes to copy
